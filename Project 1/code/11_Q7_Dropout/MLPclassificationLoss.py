...
def MLPclassificationLoss(Weights, X, y, penalty):
    ...
    # Output Weights
    gOutput = np.dot(np.concatenate((bias, fp[-1]), axis=1).T, err) + penalty * outputWeights
    err = np.multiply(ip[-1] != 0, np.multiply(sech_square(ip[-1]), np.dot(err, outputWeights[1:, :].T)))
    # Hidden Weights
    gHidden = [0] * len(hiddenWeights)
    for h in range(len(hiddenWeights) - 1, -1, -1):
        gHidden[h] = np.dot(np.concatenate((bias, fp[h]), axis=1).T, err)
        err = np.multiply(ip[h] != 0,
                          np.multiply(sech_square(ip[h]),
                                      np.dot(err, hiddenWeights[h][1:, :].T))) + penalty * hiddenWeights[h]
    # Input Weights
