...
for it in range(0, maxIter):
    if it == 0 or (it + 1) % round(maxIter / interval) == 0:
        _, yhat, _, _ = MLPclassificationPredict(Weights, Xvalid, False)
        _, ytrain, _, _ = MLPclassificationPredict(Weights, X, False)
        # Early Stopping
...
_, yhat, _, _ = MLPclassificationPredict(Wbest, Xtest, False)
...
f_prev = 0
it = 1
yhat, _, ip, fp = MLPclassificationPredict(Weights, X, False)
bias = np.ones((n, 1))
yhat = yhat.T - np.max(yhat.T, axis=0)
s = np.exp(yhat)
yhat = (s / np.sum(s, axis=0)).T
f = fine_tune(yhat, yExpanded, Weights[2], penalty)
while (fine_tune(yhat, yExpanded, Weights[2], penalty) - f_prev) ** 2 >= 1e-5:
    f_prev = fine_tune(yhat, yExpanded, Weights[2], penalty)
    g = np.dot(np.concatenate((bias, fp[-1]), axis=1).T, yhat - yExpanded) + penalty * Weights[2]
...
plt.plot(range(round(maxIter / interval), maxIter + round(maxIter / interval), round(maxIter / interval)), verr, label = 'Validation')
plt.plot(range(round(maxIter / interval), maxIter + round(maxIter / interval), round(maxIter / interval)), trerr, label = 'Training')
plt.legend()
