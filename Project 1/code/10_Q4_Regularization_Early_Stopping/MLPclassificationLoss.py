...
def MLPclassificationLoss(Weights, X, y, penalty, it):
    X = np.atleast_2d(X)
    y = np.atleast_2d(y)
    ...
    # Output Weights
    if it > 2000:
        while (fine_tune(yhat, y, outputWeights, penalty) - f_prev) ** 2 >= 1e-5:
            f_prev = fine_tune(yhat, y, outputWeights, penalty)
            g = np.dot(np.concatenate((bias, fp[-1]), axis=1).T, yhat - y) + penalty * outputWeights
