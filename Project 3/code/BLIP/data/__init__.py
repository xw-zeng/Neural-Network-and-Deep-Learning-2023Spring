...
from data.coco_DCC_dataset import coco_DCC_train, coco_DCC_caption_eval
...
def create_dataset(dataset, config,vf='',tf='', min_scale=0.5):
    ...
    elif dataset=='caption_coco_DCC':   
        train_dataset = coco_DCC_train(transform_train, config['image_root'], config['ann_root_DCC'], prompt=config['prompt'])
        val_dataset = coco_DCC_caption_eval(transform_test, config['image_root'], config['ann_root_DCC'], 'val',vf,tf)
        test_dataset = coco_DCC_caption_eval(transform_test, config['image_root'], config['ann_root_DCC'], 'test',vf,tf)   
        return train_dataset, val_dataset, test_dataset
