...
from tools.sentence_gen_tools.coco_eval import score_dcc
...
class RewardCriterion(nn.Module):
    ...
    def forward(self, seq, bn_seq, fg_seq, seqLogprobs, bnLogprobs, fgLogprobs, reward):
        ...
        seq_mask = torch.cat((seq.new(seq.size(0), 1).fill_(1).byte(), seq.gt(0)[:, :-1]), 1).view(
            -1)  # & fg_seq.eq(0)).view(-1)
...
class LMCriterion(nn.Module):
    ...
    def forward(self, txt_input, vis_input, target):
        ...
        loss = (torch.sum(txt_out) + torch.sum(vis_out)).float() / (
                torch.sum(txt_mask.data) + torch.sum(vis_mask.data)).float()
...
def noc_eval(pred, model_id, split, opt):
    ...
    gt_template_novel = './data/noc_coco/' + 'captions_split_set_%s_val_%s_novel2014.json'
    out = score_dcc(gt_template_novel, pred, noc_object, split, cache_path)
    return out
...
def language_eval(dataset, preds, model_id, split, opt):
    ...
    from pycocoevalcap.eval_2 import COCOEvalCap
