...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, state, rnn_output, det_prob, beam_fc_feats, beam_conv_feats, beam_p_conv_feats,
        ...
        for t in range(self.seq_length):
            ...
            bn_logprob, fg_logprob = self.ccr_core(vis_idx, beam_pool_feats, \
                                                   rnn_output.view(beam_size, 1, self.rnn_size), Variable(roi_labels),
                                                   beam_size, 1)
    ...
    def constraint_beam_search(self, state, rnn_output, det_prob, beam_fc_feats, beam_conv_feats, beam_p_conv_feats, \
                               beam_pool_feats, beam_p_pool_feats, beam_ppls, beam_pnt_mask, vis_offset, roi_offset,
                               tag_size, tags, opt):
        '''
        ...
        def constraint_beam_step(logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, beam_bn_seq, \
                                 beam_bn_seq_logprobs, beam_fg_seq, beam_fg_seq_logprobs, rnn_output, beam_pnt_mask,
                                 tags, vocab_size, tag_num, state):
            # INPUTS:
            # logprobsf: probabilities augmented after diversity
            # beam_size: obvious
            # t        : time instant
            ...
            for s in range(num_fsm):  # for each fsm state:
                ...
                if t == 0:
                    ...
                    rows = torch.sum(beam_seq[0][s] != 0)  # number of non-zero at first element.

                for q in range(rows):  # for each state
                    tagSet = utils.containSet(tags, tags[s])
                    for tag in tagSet:  # for each word
                        if tag == tags[s]:  # if the tag is itself
                            ...
                            for c in range(cols):  # for each column (word, essentially)
                                ...
                                candidates[tag].append(
                                    {'c': cc, 'q': q, 'p': candidate_logprob, 'r': local_logprob, 's': s})
                        else:
                            # get the index
            ...
            for s in range(len(tags)):  # for each fsm state:
                tag = tags[s]
                ...
                for vix in range(min(len(v_tag), beam_size)):
                    ...
                    for state_ix in range(len(new_state)):
                        #  copy over state in previous beam q to new beam at vix
                        new_state[state_ix][:, vix + s * beam_size] = state[state_ix][:, v['q'] + v[
                            's'] * beam_size]  # dimension one is time step

                    new_rnn_output[vix + s * beam_size] = rnn_output[
                        v['q'] + v['s'] * beam_size]  # dimension one is time step

                    # append new end terminal at the end of this beam
                    beam_seq[t, s, vix] = v['c']  # c'th word is the continuation
                    beam_seq_logprobs[t, s, vix] = v['r']  # the raw logprob here
                    beam_logprobs_sum[s, vix] = v['p']  # the new (sum) logprob along this beam
        ...
        beam_logprobs_sum = torch.zeros(num_fsm, beam_size)  # running sum of logprobs for each beam
        ...
        for t in range(self.seq_length):
            ...
            logprobs = torch.cat([decoded, prob_det], 1)
            logprobsf = logprobs.view(num_fsm, beam_size,
                                      -1).data.cpu()  # lets go to CPU for more efficiency in indexing operations
            # suppress UNK tokens in the decoding
            # logprobsf[:,logprobsf.size(1)-1] =  logprobsf[:, logprobsf.size(1)-1] - 1000
            ...
            beam_bn_seq[t] = it_bn.reshape(beam_bn_seq[t].size())
            beam_bn_seq_logprobs[t] = slp_bn.reshape(beam_bn_seq_logprobs[t].size())

            beam_fg_seq[t] = it_fg.reshape(beam_fg_seq[t].size())
            beam_fg_seq_logprobs[t] = slp_fg.reshape(beam_fg_seq_logprobs[t].size())
            ...
            for s in range(num_fsm):
                for vix in range(beam_size):
                    ...
                    if beam_seq[0, s, vix] != 0 and (beam_seq[t, s, vix] == 0 or t == self.seq_length - 1):
                        ...
                        #         if beam_seq[jj,s,vix] == 0:
                        #             idx_0 = jj
                        #             break
                        #     if idx_0 == 0: idx_0 = self.seq_length

                        #     if idx == beam_seq[idx_0-1, s, vix] or idx == beam_seq[idx_0-2, s, vix]: #last one word or last two word
                        #         skip_flag = True

                        # if skip_flag == False:
                        final_beam = {
                            'seq': beam_seq[:, s, vix].clone(),
                            'logps': beam_seq_logprobs[:, s, vix].clone(),
                            'p': beam_logprobs_sum[s, vix],
                            'bn_seq': beam_bn_seq[:, s, vix].clone(),
                            'bn_logps': beam_bn_seq_logprobs[:, s, vix].clone(),
                            'fg_seq': beam_fg_seq[:, s, vix].clone(),
                            'fg_logps': beam_fg_seq_logprobs[:, s, vix].clone(),
                        }
