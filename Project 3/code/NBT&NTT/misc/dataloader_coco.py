...
import torchtext.vocab.vectors
...
class DataLoader(data.Dataset):
    def __init__(self, opt, split='train', seq_per_img=5):
        ...
        print('DataLoader loading json file: ', 'r', opt.input_dic)
        with open(self.opt.input_dic, 'r') as f:
        ...
        det_train_path = '%s/annotations/instances_train2014.json' % opt.data_path
        det_val_path = '%s/annotations/instances_val2014.json' % opt.data_path
        ...
        for n in range(ngram, 0, -1):
            for i, s in enumerate(stem_caption):
                for j in xrange(len(s) - n + 1):
                    ...
                    if ng in self.wtod and indicator[i][j][0] == 0 and self.wtod[
                        ng] in pcats:  # make sure that larger gram not overwright with lower gram.
