...
nlp = StanfordCoreNLP(
    os.path.join('/tmp/pycharm_project_941/NeuralBabyTalk-master/prepro',
...
def main(params):
    ...
    file_dir = "/tmp/pycharm_project_941/data/noc_coco/annotations_DCC"
    for f in os.listdir(file_dir):
        split_path = os.path.join(file_dir, f)
        split_file = json.load(open(split_path, 'r'))
        if 'val_test' in f:
            for img in split_file['images']:
                split_map['test'][img['id']] = 1
        elif 'train' in f:
            for img in split_file['images']:
                split_map['train'][img['id']] = 1
        elif 'val_val' in f:
            for img in split_file['images']:
                split_map['val'][img['id']] = 1

    # create output json file
    out = {'ix_to_word': itow, 'wtod': wtod, 'wtol': wtol, 'images': []}
    count = 0
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='./coco/dataset_coco.json',
                        help='input json file to process into hdf5')
    parser.add_argument('--split', default='noc', help='different split for different task.')

    parser.add_argument('--output_dic_json', default='./noc_coco/dic_coco.json', help='output json file')
    parser.add_argument('--output_cap_json', default='./noc_coco/cap_coco.json', help='output json file')
