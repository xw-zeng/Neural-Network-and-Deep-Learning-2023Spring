...
def train(epoch, opt):
    ...
    for step in range(len(dataloader) - 1):
        ...
        if step % opt.disp_interval == 0 and step != 0:
            ...
            print(
                "step {}/{} (epoch {}), lm_loss = {:.3f}, bn_loss = {:.3f}, "
                "fg_loss = {:.3f}, rl_loss = {:.3f}, lr = {:.5f}, time/batch = {:.3f}".format(
                    step, len(dataloader), epoch, lm_loss_temp, bn_loss_temp, fg_loss_temp, rl_loss_temp,
                    opt.learning_rate, end - start))
            start = time.time()
        ...
        if iteration % opt.losses_log_every == 0:
            if tf is not None:
                with tf_summary_writer.as_default():
                    tf.summary.scalar('train_loss', loss.item(), iteration)
                    tf.summary.scalar('learning_rate', opt.learning_rate, iteration)
                    # tf.summary.scalar('scheduled_sampling_prob', model.ss_prob, iteration)
...
def eval(opt):
    ...
    for step in range(len(dataloader_val)):
        ...
        if count % 100 == 0:
            print("Evaluation function just ran for %d times" % count)
        count += 1
    ...
    if opt.inference_only:
        ...
        print(f"Welcome To Inference mode, saving scores into '{opt.checkpoint_path}'")
    ...
    if tf is not None:
        with tf_summary_writer.as_default():
            for k, v in lang_stats.items():
                tf.summary.scalar(k, v, iteration)
            tf_summary_writer.flush()
...
if __name__ == '__main__':
    tt = time.time()
    sys.stdout = open(f'/kirito/N/save/log_{tt}.txt', 'w')
    opt = opts.parse_opt()
    ...
    if opt.inference_only:
        print("Inference Mode (Caption Generation Mode)")
    else:
        print("Training Mode")
    ...
    tf_summary_writer = tf and tf.summary.create_file_writer(opt.checkpoint_path)
    infos = {}
    histories = {}
