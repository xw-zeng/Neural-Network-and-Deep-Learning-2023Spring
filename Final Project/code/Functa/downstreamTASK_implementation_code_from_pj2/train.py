...
# 从子模块导入方法类
from model import Flatten, get_simple_full_connect_model, get_simple_cnn_model, get_cnn_max_pooling_model, get_cnn_dropout_model, get_cnn_batchnorm2d_model, VGG_A, VGG18, ResNet18, ResNet18_CD_PreActivation, Identity, Identity_D, ResNet18_New
...
class Trainer:
    ...
    def intermediate_result_display(self, pretrained_model, img, channel_id = 0):
        output_final = pretrained_model.forward(img.reshape(1, 1, 32, 32))
...
if __name__ == '__main__':
    ...
    trainer.train()
    # trainer.model = torch.load("./model/ResNet18_CD_PreActivation.pth")
    ...
    output_setup = pretrained_model.out_layer_setup
    ...
    plt.figure()
    plt.subplot(1, 5, 1)
    plt.imshow(output_setup[0].permute(1, 2, 0)[:,:,channel_id].cpu().detach().numpy())
    plt.subplot(1, 5, 2)
