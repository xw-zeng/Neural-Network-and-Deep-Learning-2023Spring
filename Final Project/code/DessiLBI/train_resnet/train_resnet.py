...
if __name__ == '__main__':
    ...
    sys.stdout = open('./log_resnet.txt', 'w')
    ...
    _, fil = spa.shape
    for i in range(fil):
        label = 'ADAM: ' + layer_list[i].replace('.weight', '').replace('.0', '')
        if (i == 0) or (spa[:, i] != spa[:, i - 1]).all():
                ax3.plot(iterations, spa[:, i], color="firebrick", label=label)
    ax3.get_lines()[1].set_color('g')
