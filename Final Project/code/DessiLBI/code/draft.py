...
def shrink_group(ts, lam=1.0):  # sqrt filter size
    # shrinkage for 4-d weight(conv layer)
    ts_reshape = torch.reshape(ts, (ts.shape[0], -1))
    ts_norm = torch.div(torch.norm(ts_reshape, 2, 1), math.sqrt(ts_reshape.shape[1]))
...
class SLBI(Optimizer):
    ...
    def __init__(self, params, lr=required, kappa=1, mu=100, weight_decay=0, momentum=0.9, dampening=0, nesterov=False,
                 fc_lambda=0.1, conv_lambda=0.1, bn_lambda=0.1):
        defaults = dict(lr=lr, kappa=kappa, mu=mu, weight_decay=weight_decay, momentum=momentum, dampening=dampening,
                        nesterov=nesterov, fc_lambda=fc_lambda, conv_lambda=conv_lambda, bn_lambda=bn_lambda)
        print('*******************************************')
        for key in defaults:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'z_buffer' in param_state:
                    ...
                    elif len(p.data.size()) == 4:  # rescale the group norm!
